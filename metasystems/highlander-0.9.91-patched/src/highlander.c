	...
	meta_error e)
{
	...
	while(!http_server_shutting_down(srv)) { 
		...
		if(error)
			...
			handle_dynamic(srv, dp, request, response, e)) {
			/* We sent a dynamic page */
		}
		else if(http_server_can_read_files(srv) &&
			send_disk_file(srv, conn, request, response, e)) {
			/* We sent a file from disk */
		}
		else if(http_server_has_default_page_handler(srv) &&
			http_server_run_default_page_handler(srv, request, response, e)) {
			/* We sent a page from the default handler */
		}
		else {
			/* We didn't find a way to respond to the request */
